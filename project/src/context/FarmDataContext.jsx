...
export function FarmDataProvider({ children }) {
  ...
  const updateAnimal = async (id, updatedAnimal) => {
    if (user && user.isDemo) {
      ...
      if (!res._error) setAnimals(animals.map(animal => animal.id === id ? res : animal));
    }
  };
  ...
  const updateCrop = async (id, updatedCrop) => {
    if (user && user.isDemo) {
      ...
      if (!res._error) setCrops(crops.map(crop => crop.id === id ? res : crop));
    }
  };
  ...
  const updateTask = async (id, updatedTask) => {
    if (user && user.isDemo) {
      ...
      if (!res._error) setTasks(tasks.map(task => task.id === id ? res : task));
    }
  };
  ...
  const updateInventoryItem = async (id, updatedItem) => {
    if (user && user.isDemo) {
      ...
      if (!res._error) setInventory(inventory.map(item => item.id === id ? res : item));
    }
  };
  ...
  const updateExpense = async (id, updatedExpense) => {
    if (user && user.isDemo) {
      ...
      if (!res._error) setExpenses(expenses.map(expense => expense.id === id ? res : expense));
    }
  };
  const deleteExpense = async (id) => {
    if (user && user.isDemo) {
      ...
      if (!res._error) setExpenses(expenses.filter(expense => expense.id !== id));
    }
  };
